...
出于个人习惯偏好使用 LaTeX 进行论文排版，在撰写毕业论文期间整理了这套模板。模板整体框架参考了**厦门大学 LaTeX 论文模板**，并根据厦门理工学院的格式要求进行了适配和修改。

为了简化使用和提高兼容性，模板中的部分内容（如封面、声明页等）采用了**插入 PDF 页面**的方式实现——只需按照学校 Word 模板填写相应页面，导出为 PDF 后替换即可，无需担心格式差异问题。
...
- **编译引擎**：XeLaTeX（中文支持）
- **推荐编辑器**：VS Code + LaTeX Workshop 插件 / TeXstudio / Overleaf（付费）
...
### Q4: Overleaf 上能用吗？
可以，但需要上传所需的字体文件，并确保编译器设置为 XeLaTeX，但是学位论文通常很大，免费模式下无法完成编译。
